...
/*   Updated: 2023/05/22 17:49:48 by shinfray         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_print_map(t_fdf *s_fdf)
{
	...
	while (row-- > 0)
	{
		while (column-- > 1)
		{
			ft_draw_line(&(s_fdf->s_mlx_data.s_img), \
					ft_isometric((s_fdf->s_map_data.s_map)[i], \
					&(s_fdf->s_isometric_data)), \
					ft_isometric((s_fdf->s_map_data.s_map)[i + 1], \
					&(s_fdf->s_isometric_data)));
			++i;
		}
		++i;
	...
	while (column-- > 0)
	{
		while (row-- > 1)
		{
			ft_draw_line(&(s_fdf->s_mlx_data.s_img), \
				ft_isometric((s_fdf->s_map_data.s_map)[i], \
				&(s_fdf->s_isometric_data)), \
				ft_isometric((s_fdf->s_map_data.s_map)[i + s_fdf->s_map_data.total_column], \
				&(s_fdf->s_isometric_data)));
